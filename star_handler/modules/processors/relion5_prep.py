...
class Relion5PrepProcessor(BaseProcessor):
    ...
    def __init__(self,
                 ...
                 particle_series_dir: Union[str, Path] = 'particleseries',
        ...
        Args:
            prefix (str): The prefix for input files and to add to data.
            input_dir (Union[str, Path], optional): Directory containing input files. Defaults to '.'.
            particle_series_dir (Union[str, Path], optional): Path to the particle series directory. Defaults to 'particleseries'.
            output_dir (Union[str, Path], optional): Directory to save output files. Defaults to '.'.
            combine_prefix (str, optional): Prefix for combined star files. Defaults to 'combine'.
        """
        super().__init__()
        self.prefix = prefix
        input_dir = Path(input_dir)
        self.tomograms_star_path = input_dir / f"{prefix}_tomograms.star"
        self.particles_star_path = input_dir / f"{prefix}.star"
        self.optimization_path = input_dir / f"{prefix}_optimization_set.star"
        self.particle_series_dir = Path(particle_series_dir)
    ...
    def process(self):
        ...
        """
        self.logger.info("Starting RELION 5 preparation workflow...")
    ...
    def _backup_files(self):
        ...
        for file_path in files_to_backup:
            if file_path.exists() and backup_dir / file_path.name not in backup_dir.iterdir():
                try:
    ...
    def _rename_particle_series(self):
        ...
        if self.particle_series_dir.exists() and self.particle_series_dir.is_dir():
            shutil.move(str(self.particle_series_dir), str(new_name))
            self.logger.info(f"Renamed '{self.particle_series_dir}' to '{new_name}'")
        else:
            self.logger.warning(f"Directory not found, skipping rename: {self.particle_series_dir}")
